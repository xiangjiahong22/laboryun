...
Calendar.prototype = {
	...
	_init: function(config) {
		...
		/**
		 * 是否使用兼容模式
		 * @name Calendar.isComp
		 * @type Boolean
		 */		
		this.isComp = config.isComp || false;
		/**
		 * 日历容器
	...
	_create: function() {
		...
		if(this.triggerNode.tagName.toUpperCase() === "INPUT") {
			...
			//	this.triggerNode.style.position 	  = "absolute";
				...
				this.triggerNodeParent.style.display  = "inline";
				...
				//modify by xurb jquery兼容模式
				if(this.isComp){
				   this.triggerNodeParent.style.width    = this.triggerNode.offsetWidth || $('#'+this.triggerNode.id).outerWidth(true) + "px";
				   this.triggerNodeParent.style.height   = this.triggerNode.offsetHeight || $('#'+this.triggerNode.id).outerHeight(true) + "px";
				}
